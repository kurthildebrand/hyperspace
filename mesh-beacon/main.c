...
LOG_MODULE_REGISTER(mesh_beacon, LOG_LEVEL_INF);
// LOG_MODULE_REGISTER(mesh_beacon, LOG_LEVEL_DBG);
...
void handle_prefix_add(struct net_mgmt_event_callback*, uint32_t, struct net_if*);
...
static struct net_mgmt_event_callback handle_prefix_add_cb;
...
void main(void)
{
	...
	net_mgmt_init_event_callback(&handle_prefix_add_cb, handle_prefix_add, NET_EVENT_IPV6_PREFIX_ADD);
	net_mgmt_add_event_callback(&handle_prefix_add_cb);
...
void handle_prefix_add(
	struct net_mgmt_event_callback* cb,
	uint32_t nm_event,
	struct net_if* iface)
{
	if(nm_event == NET_EVENT_IPV6_PREFIX_ADD)
	{
		k_thread_start(&tid_update_hyperspace_coords);
	}
}
...
void update_hyperspace_coords(void* p1, void* p2, void* p3)
{
	...
	while(1)
	{
		...
		LOG_INF("sent udp HYPR update %d: %s", ret, json_str);
	}
}
