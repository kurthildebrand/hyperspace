...
LOG_MODULE_REGISTER(net_coap_server_sample, LOG_LEVEL_INF);
 ...
 * @brief		Add the COAP port and all local nodes mcast address. */
static int coap_join_mcast_group(void)
{
...
/* coap_start_server ****************************************************************************//**
 * @brief		Opens a UDP socket listening for COAP packets from any address and directed to the
 * 				COAP port. */
static int coap_start_server(void)
{
	int r;
...
/* retransmit_request ***************************************************************************//**
 * @brief		Timeout work to retransmit a pending COAP packet. */
 ...
 * @brief		Parses a coap packet and handles calling the appropriate handler. */
 ...
 * @brief		Searches for the COAP resoource assigned to the observer. */
	...
	socklen_t addr_len)
{
	...
	/* Todo: fw query parameter for image slot (image-0, image-1) */
	r = snprintk(payload, sizeof(payload), FW_MANUF "," FW_BOARD "," FW_VERSION);
	if(r < 0) {
		goto end;
	}
