...
LOG_MODULE_REGISTER(mesh_beacon_test, LOG_LEVEL_INF);
// LOG_MODULE_REGISTER(mesh_beacon_test, LOG_LEVEL_DBG);
...
{
	// const static uint8_t lladdr[] = { 0x00, 0xCF, 0xE5, 0xA0, 0x23, 0x0D, 0x67, 0xAF, };
	const static uint8_t lladdr[] = { 0x02, 0xCF, 0xE5, 0xA0, 0x23, 0x0D, 0x67, 0xAF, };

	LOG_INF("on scan cb");

	if(memcmp(ieee154_src_addr(frame), lladdr, 8) == 0)
	{
		return true;
	}
	else
