...
static bool join_coap_multicast_group(void)
{
	...
	iface = net_if_get_first_by_type(&NET_L2_GET_NAME(TSCH_L2));
	if (!iface) {
	...
	socklen_t addr_len)
{
	...
	coap_find_options(request, COAP_OPTION_URI_QUERY, options, sizeof(options) / sizeof(options[0]));
				  ...
				  const struct sockaddr *addr)
{
	...
	r = coap_pending_init(pending, response, addr, COAP_DEFAULT_MAX_RETRANSMIT);
	if (r < 0) {
		return -EINVAL;
	}
		   ...
		   struct sockaddr *addr, socklen_t addr_len)
{
	...
	return send_notification_packet(addr, addr_len, observe ? resource->age : 0, id, token, tkl, true);
}
...
not_found:
	...
	if (type == COAP_TYPE_RESET) {
		...
		o = coap_find_observer_by_addr(observers, NUM_OBSERVERS, client_addr);
		if (!o) {
...
static int process_client_request(void)
{
	...
	do {
		...
		received = recvfrom(sock, request, sizeof(request), 0, &client_addr, &client_addr_len);
		if (received < 0) {
			LOG_ERR("Connection error %d", errno);
			return -errno;
		}

		process_coap_request(request, received, &client_addr, client_addr_len);
	} while (true);
...
void coap_test(void* p1, void* p2, void* p3)
{
	int r;
